...
ALLOWED_AGENTS = ['ottnavigator', 'tivimate', 'test']
...
def init_db():
    with sqlite3.connect(DB) as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS channels (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, stream_url TEXT, logo_url TEXT, is_master INTEGER DEFAULT 0)''')
    ...
    with sqlite3.connect(DB) as conn:
        ...
        if request.method == 'POST':
            if 'add_token' in request.form:
                ...
                is_master = 1 if stream.endswith('.m3u8') else 0
                c.execute('INSERT INTO channels(name, stream_url, logo_url, is_master) VALUES (?, ?, ?, ?)', (name, stream, logo, is_master))
...
def parse_m3u_lines(lines, c):
    ...
    for line in lines:
        if line.startswith('#EXTINF:'):
            ...
            url = line.strip()
            is_master = 1 if url.endswith('.m3u8') else 0
            if name and url:
                c.execute('INSERT INTO channels(name, stream_url, logo_url, is_master) VALUES (?, ?, ?, ?)', (name, url, logo, is_master))
...
def playlist():
    ...
    with sqlite3.connect(DB) as conn:
        ...
        c.execute('INSERT INTO logs(timestamp, ip, token, user_agent, referrer) VALUES (?, ?, ?, ?, ?)',
                  (datetime.utcnow().isoformat(), ip, token, ua, request.referrer or ''))

        c.execute('SELECT name, stream_url, logo_url, is_master FROM channels')
        channels = c.fetchall()
        conn.commit()

    lines = ['#EXTM3U']
    for name, url, logo, is_master in channels:
        if is_master:
            lines.append(f'#EXTINF:-1 tvg-logo="{logo}",{name}')
            lines.append(url)  # direct m3u8 master playlist
        else:
        ...
        for (url,) in c.fetchall():
            if str(uuid.uuid5(uuid.NAMESPACE_URL, url.strip())) == str(channel_id):
                try:
                    ...
                    return Response(res.iter_content(1024), content_type=res.headers.get('Content-Type'))
                except:
